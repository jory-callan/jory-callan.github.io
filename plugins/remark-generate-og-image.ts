...
async function getInterFont(): Promise<Buffer> {
  ...
  const url =
    'https://github.com/rsms/inter/raw/v3.19/Inter%20Hinted%20for%20Windows/Web/Inter-Medium.woff2'
  ...
  try {
    ...
    // Fallback: use a built-in monospace-like setup
    console.warn('[WARN] Failed to fetch Inter font from CDN, using embedded fallback.')
    return readFileSync('plugins/og-template/Inter-Regular-24pt.ttf')
  }
}
...
async function getSatoriOptions(): Promise<SatoriOptions> {
  if (satoriOptionsCache) return satoriOptionsCache
...
async function generateOgImage(
  ...
  try {
    ...
    const svg = await satori(node, await getSatoriOptions())
